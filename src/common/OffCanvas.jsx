...
const OffCanvas = () => {
  ...
  return (
    <>
      <div
        className={`side-overlay ${offCanvas ? "show" : ""}`}
        onClick={() => {
      ...
      >
        ...
        <ul className="address-list mt-5">
          {offCanvasInfos.map((offCanvasInfo, index) => {
            return (
              <li
                className="address-list__item flx-align flex-nowrap"
                key={index}
              >
                ...
                <div className="address-list__content">
                  {Array.isArray(offCanvasInfo.link) ? (
                    offCanvasInfo.link.map((link, linkIndex) => (
                      <Link
                        to={`${link}${offCanvasInfo.text[linkIndex]}`}
                        className="address-list__text"
                        key={linkIndex}
                      >
